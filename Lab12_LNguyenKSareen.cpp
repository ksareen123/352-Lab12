/*****************************************************************************
*                                                                            *
*       Source title:   Lab12_LNguyenKSareen.CPP
...
#include "config.h"
#include "testmain.h"
#include "Lab12_LNguyenKSareen.h"
...
// file.
//
CLab12_LNguyenKSareen theApp(ProductDatasheetFlow);
...
extern "C" DLL_EXPORT int ETS_PinMap_FirstSite = 0;
	DECLARE_PINMAP
		ADD_PIN (TMU4, "", NAME_QTMU)
		ADD_PIN (TMU3, "", NAME_QTMU)
		ADD_PIN (TMU2, "", NAME_QTMU)
		ADD_PIN (TMU1, "", NAME_QTMU)
...
// Caution:: The following group definitions are filled in by the PinMap Editor
	DECLARE_GROUPMAP
		ADD_GROUP ("TMUPINS", "", TMUPINS, NAME_QTMU, TMU4, TMU3, TMU2, TMU1),
		ADD_GROUP ("TMU02", "", TMU02, NAME_QTMU, TMU3, TMU4),
		ADD_GROUP ("TMU01", "", TMU01, NAME_QTMU, TMU1, TMU2),
...
BOOL CLab12_LNguyenKSareen::UserInit( void )
{
    //////////////////////////////////////////////////////////////////////////
	...
	SetGlobalSwitchName( GP10, "Continuity");
	SetGlobalSwitchName( GP2, "ICC");

	dpinvecload( "Lab12_LNguyenKSareen.evd" ); // load vector sheet so digital pins can use machine resources
...
BOOL CLab12_LNguyenKSareen::UserExit( void )
{
    return TRUE;
}
...
BOOL CLab12_LNguyenKSareen::OnTestInit( void )
{
    //////////////////////////////////////////////////////////////////////////
...
BOOL CLab12_LNguyenKSareen::OnTestExit( void )
{
    return TRUE;
}
...
BOOL CLab12_LNguyenKSareen::OnTestStartup( void )
{
...
BOOL CLab12_LNguyenKSareen::OnTestCompletion( void )
{	
...
BOOL CLab12_LNguyenKSareen::OnLogData( void )
{
    int PassFail;
...
BOOL CLab12_LNguyenKSareen::OnFailSite( int site )
{
	if (site == MS_SITE0) {
		apuset( nOE, APU_VIOFF, 0.0, APU_30V, APU_1MA, PIN_TO_VI );
		apuset( DIR, APU_VIOFF, 0.0, APU_30V, APU_1MA, PIN_TO_VI );
...
int CLab12_LNguyenKSareen::OnSot(int SotValue)
{
    return SotValue;
}
...
BOOL CLab12_LNguyenKSareen::OnUtilError(int errnum)
{
    return TRUE;
}
...
BOOL CLab12_LNguyenKSareen::OnInterlock(ULONG intlocks)
{
    return TRUE;
}
...
ETS_PRGFLOW_FUNC Dig_Sys_TProp( int DSIndex, LPCTSTR TestLabel )
{
	...
	for (i = 500; i > 200; i--) {
		failures = 0;
		dpintimeset("A_PINS", "B2A", "DTS", DPIN_TS_NORMAL, DPIN_RZ, 200, DPIN_IGNORE_TIMING, i, DPIN_IGNORE_TIMING, MS_SITE0);
		mcurun(MCU_NOANALOG, "B2A",1);
		failures = dpingetfailpins( "digPins", DPIN_ALL_PINS, &FailPins, MS_SITE0 );
		if (failures) { //if failures > 0, it becomes logical 1
			failtime = i;
			break;
		}
	}
...
ETS_PRGFLOW_FUNC Static_TProp( int DSIndex, LPCTSTR TestLabel )
{
	...
	sp100set(PWR, SP_FV, 5, SP_10V, SP_20MA);
	apuset(DIR, APU_FV, 0, APU_30V, APU_100MA, PIN_TO_VI);
	apuset(nOE, APU_FV, 0, APU_30V, APU_100MA, PIN_TO_VI);
	...
	mcumode( MCU_CH0, MCU_CLK, 50, MCU_DUT, MCU_TRIGNORMAL, 0.0 );

	//set receiver to all digital pins
	dpinrcvset( "digPins", DPIN_RCV_IO, 2.4, 2.0, MS_ALL ); 

	//initialize and set qtmu mode to timer
	qtmumode( TMUPINS, QTMU_TIMER, QTMU_ARMOUTOFF );

	//set bus to tristate for listening
	dpinforce("A_PINS", "ZZZZZZZZ", 5, 0, MS_ALL );

	//set bus to free running clock for the first 2 b pins
	dpinforce("B_PINS", "000000CC", 5, 0, MS_ALL );

	//start at b1, b2 receive high
	qtmustart( TMU01, "B1,B2", QTMU_RCVH, QTMU_POS, 0, QTMU_NOFILTER, 0.0 );

	//stop at a1, a2 receive low
	qtmustop( TMU01, "A1,A2", QTMU_RCVL, QTMU_POS, 0, QTMU_NOFILTER, 0.0 );

	//arm qtmu
	qtmuarm( TMU01, "AUTO", 2, QTMU_EDGE, QTMU_POS );

	//measure time for results
	qtmumt(TMU01, QTMU_READTIME, 60000);
	groupgetresults( Time1, 2 );

	//log bits 2 and 1
	msLogResultAll( DSIndex++, Time1, 2 , 0, 2 );
	//msLogResultAll( DSIndex++, Time2, 2 , 0, 2 );

	//set tmu off
	qtmustart( TMU01, "OFF", QTMU_RCVH, QTMU_POS, 0, QTMU_NOFILTER, 0.0 );
	qtmustop( TMU01, "OFF", QTMU_RCVL, QTMU_POS, 0, QTMU_NOFILTER, 0.0 );

//NEXT BITS

//set bus to free running clock for the first 2 b pins
	dpinforce("B_PINS", "0000CC00", 5, 0, MS_ALL );

	//start at b1, b2 receive high
	qtmustart( TMU01, "B3,B4", QTMU_RCVH, QTMU_POS, 0, QTMU_NOFILTER, 0.0 );

	//stop at a1, a2 receive low
	qtmustop( TMU01, "A3,A4", QTMU_RCVL, QTMU_POS, 0, QTMU_NOFILTER, 0.0 );
...
//set bus to free running clock for the first 2 b pins
	dpinforce("B_PINS", "00CC0000", 5, 0, MS_ALL );

	//start at b1, b2 receive high
	qtmustart( TMU01, "B5,B6", QTMU_RCVH, QTMU_POS, 0, QTMU_NOFILTER, 0.0 );

	//stop at a1, a2 receive low
	qtmustop( TMU01, "A5,A6", QTMU_RCVL, QTMU_POS, 0, QTMU_NOFILTER, 0.0 );
	...
	//measure time for results
	qtmumt(TMU01, QTMU_READTIME, 60000);
	groupgetresults( Time3, 2 );

	//log bits 2 and 1
	msLogResultAll( DSIndex++, Time3, 2 , 0, 2 );
...
//set bus to free running clock for the first 2 b pins
	dpinforce("B_PINS", "CC000000", 5, 0, MS_ALL );

	//start at b1, b2 receive high
	qtmustart( TMU01, "B7, B8", QTMU_RCVH, QTMU_POS, 0, QTMU_NOFILTER, 0.0 );

	//stop at a1, a2 receive low
	qtmustop( TMU01, "A7, A8", QTMU_RCVL, QTMU_POS, 0, QTMU_NOFILTER, 0.0 );
	...
	//measure time for results
	qtmumt(TMU01, QTMU_READTIME, 60000);
	groupgetresults( Time4, 2 );

	//log bits 2 and 1
	msLogResultAll( DSIndex++, Time4, 2 , 0, 2 );
...
ETS_PRGFLOW_FUNC Continuity( int DSIndex, LPCTSTR TestLabel )
{
	if(!GetUserSwitch(GP10)) {
		return (msSiteStat( MS_ALL) ); // if this hasn't been chosen, exit
	}
...
//TEST 1 - B to A Transmission
	...
	dpindisconnect( MS_ALL, "A_PINS");
	dpinviset("B_PINS", DPIN_FV, 5.0, DPIN_10V, DPIN_512UA, 8.0, -2.0, MS_SITE0);
	lwait(1000);
	sp100mi(PWR, SP_MV_1X, SP_MI_1X, 4000, 13.0);
	msLogResultAll( DSIndex++ );

	//measure and log vcc with inputs low and outputs disconnected
	dpinviset("B_PINS", DPIN_FV, 0.0, DPIN_10V, DPIN_512UA, 8.0, -2.0, MS_SITE0);
	lwait(1000);
	sp100mi(PWR, SP_MV_1X, SP_MI_1X, 4000, 13.0);
	msLogResultAll( DSIndex++ );
...
//TEST 2 - A to B Transmission
	...
	dpindisconnect(MS_ALL, "B_PINS");
	dpinviset("A_PINS", DPIN_FV, 5.0, DPIN_10V, DPIN_512UA, 8.0, -2.0, MS_SITE0);
	lwait(1000);
	sp100mi(PWR, SP_MV_1X, SP_MI_1X, 4000, 13.0);
	msLogResultAll( DSIndex++ );

	//measure and log vcc with inputs low and outputs disconnected
	dpinviset("A_PINS", DPIN_FV, 0.0, DPIN_10V, DPIN_512UA, 8.0, -2.0, MS_SITE0);
	lwait(1000);
	sp100mi(PWR, SP_MV_1X, SP_MI_1X, 4000, 13.0);
	msLogResultAll( DSIndex++ );
...
//TEST 3 - Isolated inputs and outputs
	...
	dpinviset("digPins", DPIN_OFF, 0.0, DPIN_8V, DPIN_32MA, DPIN_CLAMP_OFF, DPIN_CLAMP_OFF, MS_SITE0); // turn off all digital pins from the DPU
	sp100set(PWR, SP_OFF, 0.0, SP_10V, SP_20MA); // turn off power supply pin from SP10
